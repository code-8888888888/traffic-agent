...
// maxQUICKeySize must match MAX_QUIC_KEY_SIZE in bpf/ssl_uprobe.c.
const maxQUICKeySize = 48

// maxQUICLabelLen must match MAX_QUIC_LABEL_LEN in bpf/ssl_uprobe.c.
const maxQUICLabelLen = 16

// rawQUICKeyEvent mirrors struct quic_key_event from bpf/ssl_uprobe.c.
type rawQUICKeyEvent struct {
	TimestampNS uint64
	PID         uint32
	TID         uint32
	Comm        [16]byte
	Label       [maxQUICLabelLen]byte
	LabelLen    uint32
	KeyLen      uint32
	KeyData     [maxQUICKeySize]byte
	HashType    uint32
	Variant     uint32
}
...
type Interceptor struct {
	...
	quicReader    *ringbuf.Reader // ring buffer for QUIC key events
	quicKeyStore  *QUICKeyStore
	mu            sync.Mutex
	attachedLibs  map[string]bool // library paths already probed (global)
	attachedQUIC  map[string]bool // libssl3.so paths with QUIC probes attached
...
func New(cfg config.TLSConfig) *Interceptor {
	return &Interceptor{
		...
		attachedQUIC: make(map[string]bool),
		scannedPIDs:  make(map[int]bool),
		noSSLExes:    make(map[string]bool),
		quicKeyStore: NewQUICKeyStore(),
		stopScan:     make(chan struct{}),
	}
}

// QUICKeyStore returns the key store for registering callbacks.
func (i *Interceptor) QUICKeyStore() *QUICKeyStore {
	return i.quicKeyStore
}
...
func (i *Interceptor) Start(events chan<- *types.SSLEvent) error {
	...
	// Open QUIC key events ring buffer (may be nil if BPF .o wasn't recompiled yet).
	if i.objs.SSLUprobeMaps.QuicKeyEvents != nil {
		i.quicReader, err = ringbuf.NewReader(i.objs.SSLUprobeMaps.QuicKeyEvents)
		if err != nil {
			log.Printf("[tls] QUIC key ring buffer open error: %v (QUIC interception disabled)", err)
		} else {
			go i.quicKeyReadLoop()
			log.Println("[tls] QUIC key extraction ring buffer active")
		}
	} else {
		log.Println("[tls] QUIC key ring buffer not available (BPF programs need recompilation)")
	}
...
func (i *Interceptor) attachGlobal() error {
	...
	for _, e := range entries {
		...
		// Also try attaching QUIC HKDF probes to libssl3.so for QUIC key extraction.
		for _, lib := range findSSLLibsForPID(pid) {
			// If this is NSS libssl3.so (offset-based), also attach QUIC probes.
			if lib.readOffset != 0 && strings.Contains(lib.path, "libssl3.so") {
				if err := i.attachQUICProbes(lib.path); err != nil {
					log.Printf("[tls] QUIC probes for %s: %v", lib.path, err)
				}
			}
...
// attachQUICProbes discovers and attaches QUIC HKDF probes to an NSS libssl3.so.
func (i *Interceptor) attachQUICProbes(libssl3Path string) error {
	// Guard: QUIC BPF programs must be available.
	if i.objs.UprobeQuicHkdfExpandEntry == nil || i.objs.UretprobeQuicHkdfExpandRet == nil {
		return fmt.Errorf("QUIC BPF programs not available (recompile BPF objects)")
	}

	i.mu.Lock()
	if i.attachedQUIC[libssl3Path] {
		i.mu.Unlock()
		return nil
	}
	i.mu.Unlock()

	hkdfOffset, err := findNSSQUICHkdfOffset(libssl3Path)
	if err != nil {
		return fmt.Errorf("QUIC HKDF offset discovery: %w", err)
	}

	ex, err := link.OpenExecutable(libssl3Path)
	if err != nil {
		return fmt.Errorf("open %s: %w", libssl3Path, err)
	}

	// Attach entry probe.
	entryOpts := &link.UprobeOptions{Address: hkdfOffset}
	entryLink, err := ex.Uprobe("tls13_HkdfExpandLabelRaw", i.objs.UprobeQuicHkdfExpandEntry, entryOpts)
	if err != nil {
		return fmt.Errorf("attach QUIC HKDF entry uprobe: %w", err)
	}
	i.links = append(i.links, entryLink)

	// Attach return probe.
	retOpts := &link.UprobeOptions{Address: hkdfOffset}
	retLink, err := ex.Uretprobe("tls13_HkdfExpandLabelRaw", i.objs.UretprobeQuicHkdfExpandRet, retOpts)
	if err != nil {
		return fmt.Errorf("attach QUIC HKDF return uprobe: %w", err)
	}
	i.links = append(i.links, retLink)

	i.mu.Lock()
	i.attachedQUIC[libssl3Path] = true
	i.mu.Unlock()

	log.Printf("[tls] attached QUIC HKDF uprobes to %s (offset=0x%x)", libssl3Path, hkdfOffset)
	return nil
}

// quicKeyReadLoop reads QUIC key events from the ring buffer and forwards them to the key store.
func (i *Interceptor) quicKeyReadLoop() {
	for {
		record, err := i.quicReader.Read()
		if err != nil {
			if errors.Is(err, ringbuf.ErrClosed) {
				return
			}
			log.Printf("[tls] QUIC key ring buffer read error: %v", err)
			continue
		}

		ev, err := parseQUICKeyEvent(record.RawSample)
		if err != nil {
			log.Printf("[tls] QUIC key parse error: %v", err)
			continue
		}

		log.Printf("[quic-keys] extracted: pid=%d label=%q len=%d hash=%d variant=%d",
			ev.PID, ev.Label, len(ev.KeyData), ev.HashType, ev.Variant)

		i.quicKeyStore.AddEvent(ev)
	}
}

func parseQUICKeyEvent(data []byte) (*types.QUICKeyEvent, error) {
	// Layout: timestamp_ns(8) + pid(4) + tid(4) + comm(16) + label(16) +
	//         label_len(4) + key_len(4) + key_data(48) + hash_type(4) + variant(4) = 112
	const headerSize = 8 + 4 + 4 + 16 + maxQUICLabelLen + 4 + 4 + maxQUICKeySize + 4 + 4
	if len(data) < headerSize {
		return nil, fmt.Errorf("short QUIC key record: %d bytes", len(data))
	}

	off := 0
	timestampNS := binary.LittleEndian.Uint64(data[off:]); off += 8
	pid := binary.LittleEndian.Uint32(data[off:]); off += 4
	tid := binary.LittleEndian.Uint32(data[off:]); off += 4

	var comm [16]byte
	copy(comm[:], data[off:off+16]); off += 16

	var labelBuf [maxQUICLabelLen]byte
	copy(labelBuf[:], data[off:off+maxQUICLabelLen]); off += maxQUICLabelLen

	labelLen := binary.LittleEndian.Uint32(data[off:]); off += 4
	keyLen := binary.LittleEndian.Uint32(data[off:]); off += 4

	var keyData [maxQUICKeySize]byte
	copy(keyData[:], data[off:off+maxQUICKeySize]); off += maxQUICKeySize

	hashType := binary.LittleEndian.Uint32(data[off:]); off += 4
	variant := binary.LittleEndian.Uint32(data[off:])

	if labelLen > maxQUICLabelLen {
		labelLen = maxQUICLabelLen
	}
	if keyLen > maxQUICKeySize {
		keyLen = maxQUICKeySize
	}

	commStr := strings.TrimRight(string(comm[:]), "\x00")
	label := strings.TrimRight(string(labelBuf[:labelLen]), "\x00")

	return &types.QUICKeyEvent{
		TimestampNS: timestampNS,
		PID:         pid,
		TID:         tid,
		ProcessName: commStr,
		Label:       label,
		KeyData:     append([]byte(nil), keyData[:keyLen]...),
		HashType:    hashType,
		Variant:     variant,
	}, nil
}
